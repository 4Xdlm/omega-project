 ...
 */
export function canonicalEncode(obj) {
    ...
    for (const key of sortedKeys) {
        ...
        if (value === null) {
            ...
            if (Array.isArray(value)) {
                encodedValue = '[' + value.map(v => typeof v === 'object' && v !== null
                    ? canonicalEncode(v)
                    : JSON.stringify(v)).join(',') + ']';
            }
            else {
                encodedValue = canonicalEncode(value);
            }
        }
        else {
 ...
 */
export function verifyFactHash(fact) {
 ...
 * @returns Hash SHA-256
 */
export function hashAuditEntry(entry, previousHash) {
    const hashable = {
        id: entry.id,
 ...
 */
export function hashExport(facts, auditTrail, snapshotHash) {
 ...
 * @returns ID unique
 */
export function generateFactId(input, timestamp) {
 ...
 */
export function resetFactIdCounter() {
    factIdCounter = 0;
}
/**
 ...
 * @returns ID unique
 */
export function generateSnapshotId(rootHash, timestamp) {
 ...
 * @returns ID unique
 */
export function generateAuditId(action, factId, timestamp) {
 ...
 * @returns ID unique
 */
export function generateConflictId(existingId, subject, timestamp) {
