...
export class CanonStore {
  ...
  private readonly factsBySubject: Map<string, Set<string>> = new Map();
  private readonly factsByKey: Map<string, string[]> = new Map(); // subject:predicate -> fact ids
  private readonly auditTrail: AuditEntry[] = [];
  private readonly conflicts: Map<string, FactConflict> = new Map();
  
  // État
  private lastHash: string = HASH_CONFIG.GENESIS_HASH;
  private lastAuditHash: string = HASH_CONFIG.GENESIS_HASH;
  ...
  update(
    ...
    const factWithoutHash: Omit<Fact, 'hash'> = {
      id: generateFactId(
        { ...existing, type: existing.type, source: newSource },
        now
      ),
      type: existing.type,
    ...
    const newFact: Fact = { ...factWithoutHash, hash };
  ...
  resolveConflict(
    ...
    if (choice === 'existing') {
      ...
      const resolvedConflict: FactConflict = {
        ...conflict,
  ...
  getMetrics(): CanonMetrics {
    ...
    const bySource: Record<FactSource, number> = {
      [FactSource.USER]: 0,
      [FactSource.TEXT]: 0,
    ...
    let lastModified: string = HASH_CONFIG.GENESIS_HASH;
    ...
    for (const fact of this.facts.values()) {
      ...
      switch (fact.status) {
        case FactStatus.ACTIVE: active++; break;
        case FactStatus.ARCHIVED: archived++; break;
        case FactStatus.DELETED: deleted++; break;
        case FactStatus.CONFLICTED: conflicted++; break;
      }
  ...
  verifyIntegrity(): { valid: boolean; errors: string[] } {
    ...
    let previousHash: string = HASH_CONFIG.GENESIS_HASH;
  ...
  private makeKey(subject: string, predicate: string): string {
    return `${subject}:${predicate}`;
  }

  private validateInput(input: CreateFactInput): CanonError | null {
  ...
  private createFact(input: CreateFactInput, timestamp: string): CanonResult<Fact> {
    ...
    const hash = hashFact(factWithoutHash);
    const fact: Fact = { ...factWithoutHash, hash };
  ...
  private storeFact(fact: Fact): void {
  ...
  private archiveFact(fact: Fact, timestamp: string, actor: string): void {
  ...
  private getActiveFact(ids: string[]): Fact | null {
  ...
  private getActiveFactHashes(): string[] {
    const hashes: string[] = [];
    for (const fact of this.facts.values()) {
  ...
  private matchesFilter(fact: Fact, filter: FactFilter): boolean {
    if (filter.type && fact.type !== filter.type) return false;
    ...
    if (filter.subjectPattern) {
      ...
      if (!regex.test(fact.subject)) return false;
    }
    if (filter.predicate && fact.predicate !== filter.predicate) return false;
    if (filter.source && fact.source !== filter.source) return false;
    if (filter.status && fact.status !== filter.status) return false;
    if (filter.createdAfter && fact.metadata.createdAt <= filter.createdAfter) return false;
    if (filter.createdBefore && fact.metadata.createdAt >= filter.createdBefore) return false;
  ...
  private addAuditEntry(
    ...
    const hash = hashAuditEntry(
      { id, timestamp, action, factId, actor, details },
      this.lastAuditHash
    );
 ...
 */
export function createCanonStore(clock?: ClockFn): CanonStore {
