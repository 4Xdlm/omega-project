...
import { ContextAction, ContextErrorCode, comparePositions } from './types.js';
...
function hashState(position: TextPosition, elements: Map<string, ContextElement>): string {
  const sorted = Array.from(elements.values())
    .sort((a, b) => a.id.localeCompare(b.id));
  return sha256(JSON.stringify({ position, elements: sorted }));
}

function hashSnapshot(snapshot: Omit<ContextSnapshot, 'hash'>): string {
  return sha256(JSON.stringify(snapshot));
}

function generateElementId(entityRef: string, timestamp: string): string {
...
function generateHistoryId(timestamp: string): string {
  const ts = timestamp.replace(/[-:TZ.]/g, '').substring(0, 14);
...
export class ContextTracker {
  // State
  private position: TextPosition = { paragraph: 0 };
  private readonly elements: Map<string, ContextElement> = new Map();
  ...
  moveTo(newPosition: TextPosition): ContextResult<TextPosition> {
    ...
    // Record history
    this.recordHistory(ContextAction.MOVE, now, undefined, 
      JSON.stringify(oldPosition), JSON.stringify(newPosition));
  ...
  updateElement(id: string, input: UpdateElementInput): ContextResult<ContextElement> {
    ...
    this.recordHistory(ContextAction.UPDATE_ELEMENT, now, id,
  ...
  createSnapshot(label?: string): ContextResult<ContextSnapshot> {
    ...
    const snapshotWithoutHash: Omit<ContextSnapshot, 'hash'> = {
    ...
    const snapshot: ContextSnapshot = { ...snapshotWithoutHash, hash };
  ...
  rollbackTo(snapshotId: string): ContextResult<ContextState> {
    ...
    this.recordHistory(ContextAction.ROLLBACK, now, undefined,
  ...
  applyDecay(fromPosition: TextPosition, toPosition: TextPosition): void {
    ...
    for (const [id, element] of this.elements) {
      if (element.state === ElementState.EXITED) continue;
  ...
  getMetrics(): ContextMetrics {
    const elementsByState: Record<ElementState, number> = {
    ...
    const elementsByType: Record<ElementType, number> = {
    ...
    const elementsByScope: Record<ContextScope, number> = {
  ...
  private validatePosition(position: TextPosition): ContextError | null {
  ...
  private findByEntityRef(entityRef: string): ContextElement | null {
    for (const element of this.elements.values()) {
  ...
  private countByScope(scope: ContextScope): number {
    let count = 0;
    for (const element of this.elements.values()) {
  ...
  private matchesFilter(element: ContextElement, filter: ElementFilter): boolean {
    if (filter.type && element.type !== filter.type) return false;
    if (filter.state && element.state !== filter.state) return false;
    if (filter.scope && element.scope !== filter.scope) return false;
    if (filter.minWeight !== undefined && element.weight < filter.minWeight) return false;
    if (filter.entityRefPattern) {
      const regex = new RegExp(filter.entityRefPattern, 'i');
      if (!regex.test(element.entityRef)) return false;
    }
    return true;
  }

  private calculateDistance(from: TextPosition, to: TextPosition): number {
 ...
 */
export function createContextTracker(clock?: ClockFn): ContextTracker {
