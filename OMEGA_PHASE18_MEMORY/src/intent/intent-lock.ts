...
function hashIntent(intent: Omit<Intent, 'hash'>): string {
  const hashable = {
    id: intent.id,
    type: intent.type,
    state: intent.state,
...
function generateIntentId(type: IntentType, timestamp: string): string {
...
export class IntentLock {
  // Storage
  private readonly intents: Map<string, Intent> = new Map();
  private readonly transitionHistory: Map<string, StateTransition[]> = new Map();
  private readonly listeners: Set<IntentStateListener> = new Set();
  ...
  create(input: CreateIntentInput): IntentResult<Intent> {
    ...
    const intentWithoutHash: Omit<Intent, 'hash'> = {
      id,
      type: input.type,
    ...
    const intent: Intent = { ...intentWithoutHash, hash };
  ...
  complete(intentId: string, options?: CompleteIntentOptions): IntentResult<Intent> {
    ...
    const finalIntent: Intent = { ...completedIntent, hash };
  ...
  fail(intentId: string, options: FailIntentOptions): IntentResult<Intent> {
    ...
    const finalIntent: Intent = { ...failedIntent, hash };
  ...
  cancel(intentId: string): IntentResult<Intent> {
    ...
    const finalIntent: Intent = { ...cancelledIntent, hash };
  ...
  reset(intentId: string): IntentResult<Intent> {
    ...
    const finalIntent: Intent = { ...resetIntent, hash };
  ...
  retry(intentId: string): IntentResult<Intent> {
    ...
    const finalIntent: Intent = { ...retryIntent, hash };
   ...
   * INV-MEM-02: Intent jamais ambigu
   */
  verifyNoAmbiguity(): { valid: boolean; issues: string[] } {
    const issues: string[] = [];
  ...
  getMetrics(): IntentMetrics {
    ...
    const byState: Record<IntentState, number> = {
      [IntentState.IDLE]: 0,
  ...
  private validateCreateInput(input: CreateIntentInput): IntentError | null {
  ...
  private transition(intentId: string, action: IntentAction): IntentResult<Intent> {
    const intent = this.intents.get(intentId);
    if (!intent) {
      return {
        success: false,
    ...
    const finalIntent: Intent = { ...updatedIntent, hash };
  ...
  private notifyListeners(intent: Intent, previousIntent: Intent): void {
    const history = this.transitionHistory.get(intent.id);
    if (!history || history.length === 0) return;
    
    const transition = history[history.length - 1]!;
    const event: IntentStateChangeEvent = {
      intent,
      transition,
      previousIntent,
    };
 ...
 */
export function createIntentLock(clock?: ClockFn): IntentLock {
