 ...
 */
import { readFile, rename, unlink, readdir, open, } from 'fs/promises';
import { existsSync, mkdirSync } from 'fs';
import { join, dirname, basename } from 'path';
import { randomBytes } from 'crypto';
import { PersistErrorCode, PERSIST_MAGIC, PERSIST_VERSION, DEFAULT_ADAPTER_CONFIG, computeHash, validateKey, createPersistError, } from '../core/types.js';
...
export class NodeFileAdapter {
    ...
    async save(key, data, source, options) {
        ...
        if (options?.overwrite === false && existsSync(filePath)) {
            return {
                success: false,
                error: createPersistError(PersistErrorCode.WRITE_FAILED, 'File already exists and overwrite is disabled', { key, path: filePath }),
            };
        }
        // Acquérir le lock
        ...
        try {
            ...
            if (existsSync(filePath)) {
                try {
                    ...
                    const decoded = canonicalDecodeWithVerify(existing);
            ...
            if (envelopeEncoded.bytes.length > this.config.maxFileSize) {
                return {
                    success: false,
                    error: createPersistError(PersistErrorCode.STORAGE_FULL, `Data exceeds maximum file size of ${this.config.maxFileSize} bytes`, { size: envelopeEncoded.bytes.length }),
                };
            }
            ...
            if (options?.expectedHash && envelopeEncoded.hash !== options.expectedHash) {
                return {
                    success: false,
                    error: createPersistError(PersistErrorCode.HASH_MISMATCH, 'Expected hash does not match computed hash', { expected: options.expectedHash, actual: envelopeEncoded.hash }),
                };
            }
            // === ÉCRITURE ATOMIQUE ===
            ...
            return {
                success: false,
                error: createPersistError(PersistErrorCode.WRITE_FAILED, `Failed to save: ${error instanceof Error ? error.message : 'Unknown error'}`, { key, error: String(error) }),
            };
        }
        finally {
            // Relâcher le lock
    ...
    async load(key, options) {
        ...
        if (!existsSync(filePath)) {
            return {
                success: false,
                error: createPersistError(PersistErrorCode.NOT_FOUND, `Key not found: ${key}`, { key, path: filePath }),
            };
        }
        try {
            // Lire le fichier
            ...
            if (decoded.data.magic !== PERSIST_MAGIC) {
                return {
                    success: false,
                    error: createPersistError(PersistErrorCode.MAGIC_MISMATCH, 'Invalid file format: magic mismatch', { expected: PERSIST_MAGIC, actual: decoded.data.magic }),
                };
            }
            ...
            if (options?.allowMigration === false) {
                if (decoded.data.version !== PERSIST_VERSION) {
                    return {
                        success: false,
                        error: createPersistError(PersistErrorCode.VERSION_MISMATCH, 'Version mismatch and migration not allowed', { expected: PERSIST_VERSION, actual: decoded.data.version }),
                    };
                }
            }
            ...
            if (options?.verify !== false) {
                ...
                if (dataEncoded.hash !== decoded.data.dataHash) {
                    ...
                    if (options?.verify === true) {
                        return {
                            success: false,
                            error: createPersistError(PersistErrorCode.HASH_MISMATCH, 'Data hash verification failed', { expected: decoded.data.dataHash, actual: dataEncoded.hash }),
                        };
                    }
                }
            }
            ...
            if (error instanceof SyntaxError) {
                return {
                    success: false,
                    error: createPersistError(PersistErrorCode.PARSE_FAILED, 'Failed to parse JSON', { key, error: error.message }),
                };
            }
            return {
                success: false,
                error: createPersistError(PersistErrorCode.READ_FAILED, `Failed to load: ${error instanceof Error ? error.message : 'Unknown error'}`, { key, error: String(error) }),
            };
        }
    }
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    async list(prefix) {
        try {
            ...
            return {
                success: false,
                error: createPersistError(PersistErrorCode.READ_FAILED, `Failed to list: ${error instanceof Error ? error.message : 'Unknown'}`, { error: String(error) }),
            };
        }
    }
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    async delete(key) {
        ...
        try {
            ...
            const deletedPath = join(dirname(filePath), `${DELETED_PREFIX}${Date.now()}_${basename(filePath)}`);
            ...
            return {
                success: false,
                error: createPersistError(PersistErrorCode.WRITE_FAILED, `Failed to delete: ${error instanceof Error ? error.message : 'Unknown'}`, { key, error: String(error) }),
            };
        }
    }
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    async acquireLock(lockPath) {
        ...
        while (Date.now() - startTime < this.config.lockTimeout) {
            try {
                ...
                return {
                    success: false,
                    error: createPersistError(PersistErrorCode.LOCK_FAILED, `Failed to acquire lock: ${error instanceof Error ? error.message : 'Unknown'}`, { lockPath }),
                };
            }
        }
        return {
            success: false,
            error: createPersistError(PersistErrorCode.LOCK_TIMEOUT, 'Lock acquisition timeout', { lockPath, timeout: this.config.lockTimeout }),
        };
    }
    /**
     * Relâche un lock
...
// FACTORY
// ═══════════════════════════════════════════════════════════════════════════════
export function createNodeFileAdapter(config) {
    return new NodeFileAdapter(config);
}
//# sourceMappingURL=node-file-adapter.js.map