 ...
 */
export function canonicalEncodeWithHash(value: unknown): {
  bytes: Buffer;
  json: string;
  hash: string;
} {
 ...
 */
export function canonicalDecode<T>(json: string): T {
 ...
 */
export function canonicalDecodeWithVerify<T>(
  bytes: Buffer,
  expectedHash?: string
): { data: T; hash: string; verified: boolean } {
  const actualHash = computeHash(bytes);
  const json = bytes.toString('utf8');
  const data = canonicalDecode<T>(json);
 ...
 */
export function isCanonicalJson(json: string): boolean {
  try {
 ...
 */
export function verifyJsonHash(json: string, expectedHash: string): boolean {
  const bytes = Buffer.from(json, 'utf8');
  const actualHash = computeHash(bytes);
