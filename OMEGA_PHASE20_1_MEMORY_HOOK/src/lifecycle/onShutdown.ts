...
import type { IMemoryService, SaveSnapshotResult, MemoryResult } from '../memory-service.types.js';
...
): Promise<MemoryResult<ShutdownResult>> {
...
function determineShouldSave(
  config: MemoryPolicyConfig,
  options?: ShutdownOptions
): boolean {
  // Force overrides
  if (options?.skipSave) return false;
  if (options?.forceSave) return true;
...
async function cleanupOldSnapshots(
  ...
  if (config.maxSnapshots <= 0) return 0;
  ...
  if (snapshots.length <= config.maxSnapshots) return 0;
  ...
  for (const key of toDelete) {
    ...
    if (key === config.defaultSnapshotKey) continue;
...
export function isSaveAllowed(config: MemoryPolicyConfig): boolean {
