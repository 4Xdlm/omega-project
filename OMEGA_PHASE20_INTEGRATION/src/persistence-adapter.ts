...
export class PersistenceAdapter {
  ...
  async save<T>(key: string, data: T): Promise<PersistResult<SaveResult>> {
    if (!validateKey(key)) {
    ...
    try {
      ...
      // Create envelope
      const envelope: PersistedData<T> = {
  ...
  async load<T>(key: string): Promise<PersistResult<LoadResult<T>>> {
    if (!validateKey(key)) {
      return { success: false, error: `Invalid key: ${key}` };
    }
    ...
    try {
      ...
      const envelope = JSON.parse(json) as PersistedData<T>;
  ...
  async list(prefix?: string): Promise<PersistResult<string[]>> {
    try {
      if (!existsSync(this.basePath)) {
  ...
  async delete(key: string): Promise<PersistResult<boolean>> {
    if (!validateKey(key)) {
      return { success: false, error: `Invalid key: ${key}` };
    }
