...
export function getFieldValue(fact: CanonFact, field: FactField): unknown {
  switch (field) {
...
function stringEquals(actual: unknown, expected: unknown): boolean {
  if (actual === null || actual === undefined) return false;
  return String(actual).toLowerCase() === String(expected).toLowerCase();
}

function stringNotEquals(actual: unknown, expected: unknown): boolean {
  return !stringEquals(actual, expected);
}

function stringContains(actual: unknown, search: unknown): boolean {
  if (actual === null || actual === undefined) return false;
  return String(actual).toLowerCase().includes(String(search).toLowerCase());
}

function stringNotContains(actual: unknown, search: unknown): boolean {
  return !stringContains(actual, search);
}

function stringStartsWith(actual: unknown, prefix: unknown): boolean {
  if (actual === null || actual === undefined) return false;
  return String(actual).toLowerCase().startsWith(String(prefix).toLowerCase());
}

function stringEndsWith(actual: unknown, suffix: unknown): boolean {
  if (actual === null || actual === undefined) return false;
  return String(actual).toLowerCase().endsWith(String(suffix).toLowerCase());
}

function stringMatches(actual: unknown, pattern: unknown): boolean {
  if (actual === null || actual === undefined) return false;
  try {
...
function greaterThan(actual: unknown, threshold: unknown): boolean {
...
function greaterEqual(actual: unknown, threshold: unknown): boolean {
  const a = toNumber(actual);
  const b = toNumber(threshold);
  if (a === null || b === null) return false;
  return a >= b;
...
function lessThan(actual: unknown, threshold: unknown): boolean {
  const a = toNumber(actual);
  const b = toNumber(threshold);
  if (a === null || b === null) return false;
  return a < b;
...
function lessEqual(actual: unknown, threshold: unknown): boolean {
  const a = toNumber(actual);
  const b = toNumber(threshold);
  if (a === null || b === null) return false;
  return a <= b;
...
function between(actual: unknown, min: unknown, max: unknown): boolean {
  const a = toNumber(actual);
  const minVal = toNumber(min);
  const maxVal = toNumber(max);
  if (a === null || minVal === null || maxVal === null) return false;
...
function notExists(actual: unknown): boolean {
...
function notInList(actual: unknown, list: unknown): boolean {
