...
**Version** : V1.3.3 (V1.3.2 + metrologie 2026-06-03 : EMP-18 CENTROID_AUC_LOAO + bge-m3 radar advisory DEC-019 + genre FAIL S-1). Derive de v1-3-1.md INTACT.
...
- **FORBID-OPS-PARALLEL-COMMIT-001** : en session Cowork, l'Architecte committe en parallèle Windows-side → `git fetch`/`log` AVANT toute action repo ; un commit non synchronisé a écrasé un livrable Architecte (réparé par reset). Cf mémoire `feedback-cowork-parallel-architect-commits`.
  - **Instance 2026-05-31 (NCR RCI)** : collision de doublon — Claude Code a commité `nexus/proof/NCR-RCI-SENSOR-DEFECT.md` (hyphen, `420e9d5a`) pendant que l'Architecte créait/committait en parallèle `nexus/proof/NCR_RCI_SENSOR_DEFECT.md` (underscore, `c5e01593`) + `NCR_ECC_CONTRACT_SENSOR.md` + `NCR_LOG.md`. **Pas d'écrasement** (noms distincts, 0 conflit git) mais **2 NCR pour le même défaut**. Résolution (acteur unique anti-collision, GO Architecte) : underscore = canonique ; preuve MINAXIS_E fusionnée dedans ; doublon hyphen supprimé (`git rm`). Leçon : avant tout `git mv`/commit de gouvernance en session Cowork, `git status` immédiat pour détecter le staging parallèle.
...
# PARTIE XVII — MÉTROLOGIE DU JUGEMENT (Session 2026-06-03)

> Ajouts V1.3.3. Tous sourcés `[SSOT]` commit/tag. Branche `phase-r-dispatcher-v33`.
> Contexte : campagne S1→V4 sous mandat « on ne prend rien sans contrôle vérité mathématique ».

## 7.10 EMP-18 — CENTROID_AUC_LOAO_MANDATORY [LOI PERMANENTE — Architecte 2026-06-03]

**Énoncé** : toute métrique fondée sur des CENTROÏDES de classe (AUC, accuracy, F1, distance-au-centroïde) DOIT être évaluée en **LOAO** (leave-one-author-out) ou **LOFO** (leave-one-family-out). Un centroïde calculé sur l'ensemble complet — incluant l'item OU l'auteur testé — = **fuite de données = score NUL ET NON AVENU**. Couplage obligatoire : bootstrap clusterisé par auteur (IC95) + permutation au niveau auteur (jamais au niveau livre).

- **Origine** : V4-G3→V4-G4. AUC genre `0.839` (centroïdes full-data) → **`0.582`** en LOAO honnête. Écart +0.257 d'illusion. La fuite a failli faire construire un classifieur de genre sur une hallucination statistique.
- **État d'échec** : `CENTROID_FULLDATA_LEAK` = mesure invalide.
- `[SSOT]` commit `e61a7b5f` (ratification), `775771f8` (preuve), CLAUDE.md §H ligne 17 (v3.162.0), `docs/metrology/CENTROID_AUC_VALIDITY_RULE.md`.
- **Famille** : METRIC_HONESTY / TEST_CAUSAL (EMP-14). **revalidate_if** : jamais (loi permanente).

## 7.11 RADAR GÉOMÉTRIQUE — bge-m3 (DEC-20260603-019) [ACCEPTÉ advisory]

- **Décision** : `bge-m3` = embedding radar géométrique advisory PRINCIPAL ; `nomic-embed-text` = comparateur/fallback historique (conservé, EMP-17). Supersede la composante radar-nomic de DEC-018.
- **Preuve** (Gold-Set scellé `4388b4b6`, LOAO auteur + bootstrap 2000 + permutation) :
  - MASTER_FR vs C_formulaic_FR : bge-m3 **0.943 [0.78–0.98]** (FRANCHIT S-1) vs nomic 0.821 [0.56–0.92].
  - MASTER_EN vs C_formulaic_EN : bge-m3 0.889 vs nomic 0.86.
  - MASTER_FR vs D_pulp_réel_FR : bge-m3 0.796 vs nomic 0.84 (contraste DUR, **plafond mondial ~0.77** — Underwood/van Cranenburgh).
- **Contrainte** : advisory uniquement. **PAS de gate, PAS de SEAL, PAS de production gate** fondés seul sur bge-m3. Ne remplace pas le juge gemma4 (DEC-018) sur les cas complexes.
- `[SSOT]` commit `775771f8`, `docs/architecture/DEC-20260603-019-BGE-M3-ADVISORY-RADAR.md`.

## 7.12 COUCHE GENRE — GEL (FAIL S-1) [FREEZE scellé Tribunal 2/2]

- **Verdict** : `GENRE_SEPARABILITY_FAIL_S1`. macro-OVR-AUC LOAO honnête = **0.582**, IC95 [0.34–0.64], permutation p=0.063 → échoue les 3 critères S-1.
- Par genre (bge-m3) : FEELGOOD 0.779 (séparable) ; THRILLER 0.522 et SF 0.445 (≈ hasard).
- **Cause** : corpus/labels (88 livres FR, 21 auteurs, KB épuisé), PAS méthode ni modèle. Mesurer plus = mesurer l'auteur/la source, pas le genre.
- **FORBID** (interdits tant que corpus non élargi) : ❌ genre gate ❌ percentile intra-genre ❌ routeur genre ❌ SEAL ❌ production gate.
- **Statut conceptuel** : `GenreRelativeQuality` = validé architecturalement ; `GenreClassifier` = NON validé empiriquement → HOLD.
- **Condition de relance** : corpus genre-tagué ≥30 auteurs/genre, ≥100 livres/genre, FR/EN séparés, same-era/source (cf `docs/metrology/V4_GENRE_CORPUS_REQUIREMENTS.md`).
- `[SSOT]` commit `775771f8`/`ab69e04c`, `docs/metrology/V4_G4_GENRE_SEPARABILITY_FAIL.md`.

## 7.13 TYPE gate (fiction/non-fiction) — EVIDENCE_GAP données [HOLD]

- **Verdict** : `EVIDENCE_GAP_TYPE_GATE`. Séparation fiction/non-fiction non testable : classe non-fiction **physiquement absente** du corpus (~98 % fiction, purgée même en S1C+) ; aucune colonne TYPE ; champ `author` dégénéré sur R2_FEATURE_MATRIX.
- **Risque réel non mesurable** : biographie/histoire jugée comme « roman raté » — confirmé comme danger mais non quantifiable faute de corpus non-fiction.
- **Débloquage** : acquérir corpus non-fiction (biographie/histoire/essai ≥30 auteurs/type FR+EN).
- `[SSOT]` commit `e346ef0e`, `docs/metrology/V4A0_TYPE_AUDIT_EVIDENCE_GAP.md`.

## 7.14 STACK DE JUGEMENT OMEGA — état au 2026-06-03

| Composant | Rôle | Statut | Source |
|---|---|---|---|
| bge-m3 | Radar géométrique | Advisory principal | DEC-019 |
| nomic-embed-text | Comparateur/fallback | Conservé | DEC-018/019 |
| gemma4:31b | Juge LLM pairwise | Advisory calibré (biais position 0.50, droit au TIE) | DEC-018 |
| qwen3:32b, mistral-small | — | DISQUALIFIÉS (biais position 0.64–0.98) | S1E-B |
| Norme S-1 | Preuve | AUC≥0.80 + IC95-bas≥0.70 + permutation + LOAO/LOFO (EMP-18) | OMEGA_ABSOLUTE_PROOF_STANDARD.md |

**Plafond établi** : discrimination qualité-de-publication réelle plafonne ~0.77–0.80 = état de l'art mondial. OMEGA mesure désormais la réalité du texte, pas du bruit.
