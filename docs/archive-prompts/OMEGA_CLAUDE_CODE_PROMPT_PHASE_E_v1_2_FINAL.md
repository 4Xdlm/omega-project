...
| E5-T3 | CONTR-002 entité manquante | INV-E-CONFLICT-01 |
| E5-T4 | CONTR-003 boucle supersession | INV-E-CONFLICT-01 |
| E5-T5 | NaN dans object = FAIL | INV-E-NAN-01 |
| E5-T6 | Guard FAIL = no write | INV-E-CONFLICT-02 |
| E5-T7 | Pas d'usage !== pour sémantique | INT-E-07 |

### Golden Tests E5

**E5-GOLD-1 : CONTR-001 avec semanticEquals**
```typescript
// Deux objets JSON équivalents
const obj1 = { a: 1, b: 2 };
const obj2 = { b: 2, a: 1 }; // Ordre différent
// Pas de conflit car semanticEquals(obj1, obj2) === true
expect(guard.checkConflict(claimWithObj1, claimWithObj2)).toBe(false);
```

**E5-GOLD-2 : CONTR-001 objets différents**
```typescript
const obj1 = { a: 1 };
const obj2 = { a: 2 }; // Valeur différente
// Conflit car semanticEquals(obj1, obj2) === false
expect(guard.checkConflict(claimWithObj1, claimWithObj2)).toBe(true);
```
...
| Export | Type | Description |
|--------|------|-------------|
| `CanonQuery` | Class | API lecture |
| `getById` | Method | Par ID |
| `getAtVersion` | Method | Snapshot |
| `query` | Method | Par critères |
| `getActiveBySubjectPredicate` | Method | Lookup actif |

**Invariants** : INV-E-READ-01 à 03

---

## §7.4 GATE E6 — QUERY DETERMINISM

| ID | Test | Invariant |
|----|------|-----------|
| E6-T1 | getById déterministe | INV-E-READ-03 |
| E6-T2 | query déterministe | INV-E-READ-03 |
| E6-T3 | No scan si index valide | INV-E-READ-01 |
| E6-T4 | getAtVersion snapshot | INV-E-READ-03 |
| E6-T5 | Index invalide = incident | INV-E-READ-02 |

---

# ════════════════════════════════════════════════════════════════════════════════
#                    PARTIE VIII — MODULES PHASE 6 (INTEGRATION)
# ════════════════════════════════════════════════════════════════════════════════
...
| Export | Type | Description |
|--------|------|-------------|
| `CanonApi` | Class | Façade unifiée |
| `appendClaim` | Method | Guard → Sentinel → Write → Receipt |
| `getClaim` | Method | Query par ID |
| `queryClaims` | Method | Recherche |
| `supersedeClaim` | Method | Nouveau + supersede |

### Pipeline (spec v1.2 §8)

```
REQUEST ──► GUARD ──► SENTINEL ──► WRITE ──► RECEIPT ──► RESPONSE
             │          │          │
             ▼          ▼          ▼
           FAIL?      DENY?      FAIL?
             │          │          │
...
**Invariants** : INV-E-PIPELINE-01, INV-E-PIPELINE-02

---

## §8.2 GATE E7 — PIPELINE INTEGRITY

| ID | Test | Invariant |
|----|------|-----------|
| E7-T1 | Guard → Sentinel → Receipt ordre | INV-E-PIPELINE-01 |
| E7-T2 | Bijection writes/receipts/traces | INV-E-PIPELINE-02 |
| E7-T3 | Guard FAIL = no write | INV-E-PIPELINE-01 |
| E7-T4 | Sentinel DENY = no write | INV-E-PIPELINE-01 |
| E7-T5 | Write FAIL = rollback | INV-E-PIPELINE-01 |
| E7-T6 | Receipt mandatory pour ALLOW | INV-E-PIPELINE-02 |

---

## §8.3 GATE E-PERF — PERFORMANCE

| ID | Test | Target (ConfigSymbol) |
|----|------|-----------------------|
| PERF-T1 | getById P95 < 10ms | `P95_GETBYID_TARGET_MS` |
| PERF-T2 | query P95 < 100ms | `P95_QUERY_TARGET_MS` |
| PERF-T3 | Seed dataset | `PERF_SEED_CLAIMS_COUNT` |

---

# ════════════════════════════════════════════════════════════════════════════════
#                    PARTIE IX — AUDIT PROD (100M CLAIMS)
# ════════════════════════════════════════════════════════════════════════════════

## §9.1 Points de Vigilance Prod

**À vérifier avant déploiement production :**

| Point | Risque | Mitigation |
|-------|--------|------------|
| Index rebuild | O(n) sur 100M claims = lent | Rebuild incrémental futur |
| Segment rotation | IO intense | Async rotation |
| Memory index | RAM pour 100M entries | Memory-mapped files futur |
| Query performance | Scan si index manquant | Index obligatoire |
| Manifest corruption | Perte de données | Backup + checksum |

**Ces points sont HORS SCOPE Phase E mais documentés pour audit.**

---

# ════════════════════════════════════════════════════════════════════════════════
#                    PARTIE X — SEAL & RAPPORT FINAL
# ════════════════════════════════════════════════════════════════════════════════

## §10.1 FICHIERS À SCELLER

```typescript
const PHASE_E_FILES = [
  // Config
  'config/canon/id_config.json',
  'config/canon/segment_config.json',
  'config/canon/perf_config.json',
  'config/canon/predicate_catalog.json',
  
  // Source
  'src/canon/config-symbol.ts',
  'src/canon/types.ts',
  'src/canon/id-factory.ts',
  'src/canon/semantic-equals.ts',
  'src/canon/predicate-catalog.ts',
  'src/canon/lineage.ts',
  ...
  // Tests
  'tests/canon/config-symbol.test.ts',
  'tests/canon/types.test.ts',
  'tests/canon/id-factory.test.ts',
  'tests/canon/semantic-equals.test.ts',
  'tests/canon/predicate-catalog.test.ts',
  'tests/canon/lineage.test.ts',
  'tests/canon/segment.test.ts',
  'tests/canon/index-builder.test.ts',
  'tests/canon/guard.test.ts',
  'tests/canon/query.test.ts',
  'tests/canon/pipeline.test.ts',
  'tests/canon/perf.test.ts',
  
  // Scripts
  'scripts/verify-cd-sealed.ts',
  'scripts/gate-e1-schema.ts',
  'scripts/gate-e2-catalog.ts',
  'scripts/gate-e3-segment.ts',
  'scripts/gate-e4-index.ts',
  'scripts/gate-e5-conflict.ts',
  'scripts/gate-e6-query.ts',
  'scripts/gate-e7-pipeline.ts',
  'scripts/gate-e-perf.ts',
  'scripts/seal-phase-e.ts',
];
```

---

## §10.2 FORMAT RAPPORT FINAL

```markdown
# ═══════════════════════════════════════════════════════════════════════════════
#                    OMEGA PHASE E RAPPORT FINAL
# ═══════════════════════════════════════════════════════════════════════════════
...
|----------|---------|---------|
| CANON_SCHEMA_SPEC | v1.2.0 | 6a6298ae... |

## INTERDICTIONS VÉRIFIÉES (8)
| ID | Status |
|----|--------|
| INT-E-01 | ✅ |
| INT-E-02 | ✅ |
...
|------|--------|
| E1 - Schema Model | ✅ |
| E2 - Catalog Integrity | ✅ |
| E3 - Segment Integrity | ✅ |
| E4 - Index Rebuild | ✅ |
| E5 - Conflict Detection | ✅ |
| E6 - Query Determinism | ✅ |
| E7 - Pipeline Integrity | ✅ |
| E-PERF - Performance | ✅ |

## SEAL
| Phase | Tag | Manifest Hash |
|-------|-----|---------------|
| E | OMEGA_CANON_PHASE_E_v1.0_SEALED | XXXXXXXX... |

## COMMANDES GIT
```bash
git add .
git commit -m "feat(canon): Phase E complete [INV-E-*]"
git tag -a OMEGA_CANON_PHASE_E_v1.0_SEALED -m "Phase E Canon complete"
...
# ════════════════════════════════════════════════════════════════════════════════
#                    FIN DU PROMPT PHASE E
# ════════════════════════════════════════════════════════════════════════════════

```
╔═══════════════════════════════════════════════════════════════════════════════════════════════════════╗
║                                                                                                       ║
║   OMEGA PHASE E PROMPT v1.2 FINAL                                                                     ║
║                                                                                                       ║
║   CORRECTIONS v1.1 → v1.2 (Audit ChatGPT):                                                            ║
║   ✅ Ajout INT-E-07 explicite (interdit !== pour sémantique)                                          ║
║   ✅ Ajout R21 (semanticEquals obligatoire)                                                           ║
║   ✅ Ajout R22 (SUPERSEDES champ uniquement)                                                          ║
║   ✅ Suppression SUPERSEDES du predicate_catalog.json                                                 ║
║   ✅ Golden tests E1-GOLD-1/2/3/4 explicites                                                          ║
║   ✅ Golden tests E5-GOLD-1/2 pour CONTR-001                                                          ║
║   ✅ Test E2-T6 (pas de SUPERSEDES dans catalog)                                                      ║
║   ✅ Section §9 Audit Prod 100M claims                                                                ║
║   ✅ Test E5-T7 (pas de !== pour sémantique)                                                          ║
║                                                                                                       ║
║   Modules: 13 source + 12 tests + 10 scripts                                                          ║
║   Gates: 8 (E1-E7 + E-PERF)                                                                           ║
║   Invariants: 29+ (définis spec v1.2)                                                                 ║
║   Interdictions: 8 (INT-E-01 à INT-E-08)                                                              ║
║   Règles: 22 (R01-R22)                                                                                ║
║                                                                                                       ║
║   Standard: NASA-Grade L4 / DO-178C                                                                   ║
║   Date: 2026-01-28                                                                                    ║
║   Spec SHA-256: 6a6298aed6e50ad86d119a4e876d9119838d65d994a0ba3162cc7a5919eaeac2                       ║
║                                                                                                       ║
╚═══════════════════════════════════════════════════════════════════════════════════════════════════════╝
```
...
**FIN DU DOCUMENT OMEGA_CLAUDE_CODE_PROMPT_PHASE_E v1.2.0 FINAL**
