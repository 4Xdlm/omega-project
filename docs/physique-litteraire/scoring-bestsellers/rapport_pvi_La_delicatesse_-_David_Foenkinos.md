# RAPPORT PVI — La_delicatesse_-_David_Foenkinos
Langue : FR | Date : 2026-03-30 | Modele : MINIMAL v2 (cultural_FR)

## SCORES

| Variable | Score | Methode | Seuil critique | Statut |
|----------|-------|---------|----------------|--------|
| I | 0.1161 | PROXY-NLP-v3 | >= 0.65 | GOULOT |
| FL | 0.2270 | NLP-ROBUSTE-v2 | <= 0.25 | OK |
| Omega | 0.7200 | SEMI-AUTO | >= 0.72 | OK |
| T | 0.7190 | proxy(1-DR) | >= 0.75 | GOULOT |
| MS | 0.8539 | NLP-ROBUSTE-v3 | >= 0.85 | OK |
| N_rev | 3 | semi-auto | >= 2 | OK |
| S | 1.0000 | NLP-PARTIEL | — | — |
| U | 0.6500 | SEMI-AUTO | — | — |

## CALCULS

E_emo = 0.4303 | E_cog = 0.1851 | CE = 2.3251

R = 0.3863 | W = 0.3473 | Arc_rev = 1.00

Score etouffement FL x (1-Omega) = 0.0636

**PVI = 0.2722 | SP = 5.4/100**

## VERDICT

**Phase 1 : mort organique**

**Probabilite bestseller : 38.5% -> FAIL — pas de potentiel bestseller intrinseque**

## GOULOTS ACTIFS

- **GOULOT-I** : Protagoniste insuffisamment identifiable (actuel=0.12, seuil=0.55)
- **GOULOT-R** : Lecteur n'atteint probablement pas la fin (actuel=0.39, seuil=0.5)
- **GOULOT-W** : Transmissibilite nulle (actuel=0.35, seuil=0.5)

## TOP 3 LEVIERS

1. **FL** -0.10 -> PVI +0.1070 (+39.3%)
2. **I** +0.10 -> PVI +0.1007 (+37.0%)
3. **Omega** +0.10 -> PVI +0.0526 (+19.3%)

## DISTANCE ZONE OMEGA

| Variable | Actuel | Cible | Delta manquant | Statut |
|----------|--------|-------|----------------|--------|
| FL | 0.2270 | 0.25 | OK | OK |
| MS | 0.8539 | 0.85 | OK | OK |
| Omega | 0.7200 | 0.72 | OK | OK |
| I | 0.1161 | 0.65 | **-0.53** | MANQUE |
| T_proxy | 0.7190 | 0.75 | **-0.03** | MANQUE |
| N_rev | 3.0000 | 2 | OK | OK |

Reference Zone OMEGA: Hemingway PVI=2.441 / Fitzgerald PVI=1.699
