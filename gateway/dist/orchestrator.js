...
// ═══════════════════════════════════════════════════════════════════════════════
import { CONSTANTS, TERMINAL_STATES } from './types';
import { parseModuleKey } from './registry';
...
function isValidTransition(from, to) { return VALID_TRANSITIONS[from].includes(to); }
...
export class Orchestrator {
    ...
    audit;
    cancelled = new Set();
    constructor(moduleRegistry, moduleLoader, audit) {
    ...
    async execute(request) {
        ...
        if (!request.execution_token)
            return this.failExecution(state, 'ORCH_NO_TOKEN', 'Missing execution token');
        if (!request.pipeline_spec)
            return this.failExecution(state, 'ORCH_NO_SPEC', 'Missing pipeline spec');
        const deadline = request.pipeline_spec.constraints.max_runtime_ms;
        if (!deadline || deadline < 1000)
            return this.failExecution(state, 'ORCH_BAD_DEADLINE', 'Invalid max_runtime_ms');
        const chain = request.pipeline_spec.module_chain;
        if (!chain || chain.length === 0)
            return this.failExecution(state, 'ORCH_EMPTY_CHAIN', 'module_chain is empty');
        this.transition(state, 'INITIALIZING');
        const modules = [];
        for (const moduleKey of chain) {
            const parsed = parseModuleKey(moduleKey);
            if (!parsed)
                return this.failExecution(state, 'ORCH_MODULE_NOT_FOUND', `Invalid module key: ${moduleKey}`);
            const spec = this.moduleRegistry.get(parsed.module_id, parsed.version);
            if (!spec)
                return this.failExecution(state, 'ORCH_MODULE_NOT_FOUND', `Module not found: ${moduleKey}`);
            ...
            if (!instance)
                return this.failExecution(state, 'ORCH_MODULE_NOT_FOUND', `Cannot load module: ${moduleKey}`);
        ...
        for (let i = 0; i < modules.length; i++) {
            ...
            if (this.cancelled.has(request.execution_token))
                return this.cancelExecution(state);
            if (Date.now() >= deadlineEpoch)
                return this.timeoutExecution(state, key);
    ...
    cancel(execution_token) { this.cancelled.add(execution_token); }
    transition(state, to) {
        if (!isValidTransition(state.state, to))
            throw new OrchestratorError('ORCH_STATE_VIOLATION', `Invalid transition ${state.state} → ${to}`);
    ...
    failExecution(state, code, message, moduleError) {
        state.end_time = new Date().toISOString();
        ...
        state.error_trace = { step: `step_${state.current_step}`, code, message, details: moduleError ? JSON.stringify(moduleError) : undefined, retryable: moduleError?.retryable ?? false };
    ...
    timeoutExecution(state, currentModule) {
        state.end_time = new Date().toISOString();
        state.metrics.total_duration_ms = Date.now() - new Date(state.start_time).getTime();
    ...
    async executeWithTimeout(promise, timeoutMs, moduleKey) {
        if (timeoutMs <= 0)
            throw new OrchestratorError('ORCH_TIMEOUT', `Timeout before executing ${moduleKey}`);
        return Promise.race([promise, new Promise((_, reject) => setTimeout(() => reject(new OrchestratorError('ORCH_TIMEOUT', `Module ${moduleKey} exceeded timeout`)), timeoutMs))]);
    }
    createContext(execution_token, spec, mode, trace, deadlineEpoch) {
        const self = this;
        return { execution_token, pipeline_id: spec.pipeline_id, mode, trace, deterministic_required: spec.constraints.deterministic_required, deadline_epoch_ms: deadlineEpoch, audit: { append(event_type, payload) { self.auditAppend(event_type, execution_token, payload); } }, artifacts: { async put() { throw new Error('Not implemented'); }, async get() { throw new Error('Not implemented'); }, async verify() { throw new Error('Not implemented'); } }, rng: createDeterministicRNG(CONSTANTS.DETERMINISTIC_SEED) };
    }
    auditAppend(event_type, execution_token, payload) {
        this.audit.append({ event_type, timestamp: new Date().toISOString(), execution_token, payload });
    }
}
function createDeterministicRNG(seed) {
    let state = seed;
    function next() { state = (state * 1664525 + 1013904223) >>> 0; return state / 0xFFFFFFFF; }
    return { seed, next, nextInt(min, max) { return Math.floor(next() * (max - min + 1)) + min; } };
}
export function createOrchestrator(moduleRegistry, moduleLoader, audit) { return new Orchestrator(moduleRegistry, moduleLoader, audit); }
export class MockModuleLoader {
    modules = new Map();
    register(module_id, version, module) { this.modules.set(`${module_id}@${version}`, module); }
    async load(module_id, version) { return this.modules.get(`${module_id}@${version}`) || null; }
}
export function createPassthroughModule(id, version) { return { id, version, limits: { deterministic_safe: true }, validate: () => ({ ok: true }), run: async (ctx, input) => ({ ok: true, output: input }) }; }
export function createFailingModule(id, version, errorCode = 'MOD_EXECUTION_ERROR') { return { id, version, limits: { deterministic_safe: true }, validate: () => ({ ok: true }), run: async () => ({ ok: false, error: { code: errorCode, message: 'Module intentionally failed', retryable: false, category: 'EXECUTION' } }) }; }
export function createSlowModule(id, version, delayMs) { return { id, version, limits: { deterministic_safe: true }, validate: () => ({ ok: true }), run: async (ctx, input) => { await new Promise(r => setTimeout(r, delayMs)); return { ok: true, output: input }; } }; }
