...
export class PolicyBuilder {
  ...
  private nextPriority: number = 1000;
  ...
  allow(
    id: string,
    description: string,
    predicate: (req: PolicyCheckRequest) => boolean,
    constraints?: PolicyConstraints,
    priority?: number
  ): PolicyBuilder {
    this.rules.push({
  ...
  deny(
    ...
    description: string,
    predicate: (req: PolicyCheckRequest) => boolean,
    reason_code: PolicyReasonCode = 'POL_DEFAULT_DENY',
    priority?: number
  ): PolicyBuilder {
    this.rules.push({
      id,
   ...
   * Blacklist un caller
   */
  blacklistCaller(caller_id: string, priority: number = 9999): PolicyBuilder {
    return this.deny(
      `blacklist_${caller_id}`,
      `Blacklist caller ${caller_id}`,
      (req) => req.request.caller.id === caller_id,
      'POL_CALLER_BLACKLISTED',
      priority
    );
  }
   ...
   * Interdit un intent
   */
  forbidIntent(intent: string, priority: number = 9998): PolicyBuilder {
    return this.deny(
      `forbid_${intent}`,
      `Forbid intent ${intent}`,
      (req) => req.request.intent === intent,
      'POL_INTENT_FORBIDDEN',
      priority
    );
  }
   ...
   * Restreint un mode
   */
  restrictMode(mode: ExecutionMode, priority: number = 9997): PolicyBuilder {
    return this.deny(
      `restrict_${mode}`,
      `Restrict mode ${mode}`,
      (req) => req.environment.mode === mode,
      'POL_MODE_RESTRICTED',
      priority
    );
  }
  ...
  allowCallerType(type: CallerType, priority?: number): PolicyBuilder {
    return this.allow(
      `allow_caller_${type}`,
      `Allow caller type ${type}`,
      (req) => req.request.caller.type === type,
      undefined,
      priority
    );
  }
 ...
 */
export function createDefaultPolicy(version: string = '1.0.0'): PolicyEngine {
  return new PolicyBuilder()
    .setVersion(version)
    .allow(
      'allow_system',
      'Allow all SYSTEM callers',
      (req) => req.request.caller.type === 'SYSTEM',
      undefined,
      1000
    )
    .allow(
      'allow_user_prod',
      'Allow USER callers in PROD',
      (req) => req.request.caller.type === 'USER' && req.environment.mode === 'PROD',
      { trace_required: true },
      900
    )
    .allow(
      'allow_pipeline',
      'Allow PIPELINE callers',
      (req) => req.request.caller.type === 'PIPELINE',
      undefined,
      800
    )
    .build();
 ...
 */
export function createAllowAllPolicy(version: string = '1.0.0'): PolicyEngine {
  return new PolicyBuilder()
    .setVersion(version)
 ...
 */
export function createDenyAllPolicy(version: string = '1.0.0'): PolicyEngine {
  return new PolicyBuilder()
    .setVersion(version)
 ...
 */
export function createStrictPolicy(version: string = '1.0.0'): PolicyEngine {
  return new PolicyBuilder()
    .setVersion(version)
    // Blacklist patterns dangereux
    .deny(
      'deny_dangerous_intent',
      'Deny dangerous intents',
      (req) => req.request.intent.includes('__') || req.request.intent.includes('..'),
      'POL_INTENT_FORBIDDEN',
      10000
    )
    ...
    .allowWithConstraints(
      'allow_user',
      'Allow USER with constraints',
      (req) => req.request.caller.type === 'USER',
      {
    ...
    .allowWithConstraints(
      'allow_pipeline',
      'Allow PIPELINE with constraints',
      (req) => req.request.caller.type === 'PIPELINE',
      {
