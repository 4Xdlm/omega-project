...
**MAJ 2026-05-31 (WS-B2 + DEC-013)** : root cause = **PACKET_UNDERPOPULATION_CONFIRMED**. WS-B2 : packet représentatif (2 lexiques) → signature/hook saturent 100 → RCI maîtres ≈ ceiling médiane 84.54, maîtres ≥ K2. signature/hook reclassés `packet-completeness signals` (non quality). Floor 85 trop sévère (≈ médiane maîtres) ; candidat **shadow 81.7** (p25), aucun changement prod avant shadow bench. Décisions scellées dans **DEC-20260531-013** (D1-shadow / D2 / Q1 + règle INVALID_PACKET). NCR reste OPEN_DIAGNOSED jusqu'au shadow bench + décision floor Architecte.
