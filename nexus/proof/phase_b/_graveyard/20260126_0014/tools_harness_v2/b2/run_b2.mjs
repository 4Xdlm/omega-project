...
if(mode !== "RUN1" && mode !== "RUN2"){
  console.error("USAGE: node run_b2.mjs RUN1|RUN2");
  process.exit(2);
}
...
const outDir    = resolve(ROOT, "nexus/proof/phase_b/b2");
...
writeFileSync(resolve(outDir, `B2_PAYLOAD_${mode}.json`), payloadJson, "utf8");
writeFileSync(resolve(outDir, `B2_REPORT_${mode}.md`),
`# B2 REPORT (${mode})
ROOT_A: ${rootA}
...
console.log(`B2_OK ${mode} RUN_HASH=${runHash}`);
