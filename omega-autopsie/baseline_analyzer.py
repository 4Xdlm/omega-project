...
def main():
    ...
    for feat, comp in report["comparisons"].items():
        generic = comp.get("generic_expected")
        measured = comp.get("corpus_measured_median")
        delta = comp.get("generic_vs_measured_delta_pct", "—")
        verdict = comp.get("verdict", "—")
        g_str = str(generic) if generic is not None else "N/A"
        m_str = str(measured) if measured is not None else "N/A"
        print(f"  {feat:30s} générique={g_str:8} mesuré={m_str:8} "
              f"Δ={delta}%  [{verdict}]")
