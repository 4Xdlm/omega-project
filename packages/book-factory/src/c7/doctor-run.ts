...
async function main(): Promise<void> {
  ...
  const md = [
    ...
    `Avant: drifts=${rep.auditBefore.identityDrifts} physics=${rep.auditBefore.physicsSignals} chap=${rep.auditBefore.chapterSignals} ticsFS=${rep.auditBefore.ticsFailShadow} unpaid=${rep.auditBefore.seedsUnpaid} **coutures=${rep.auditBefore.brokenStitches}**`,
    `Après: drifts=${rep.auditAfter.identityDrifts} physics=${rep.auditAfter.physicsSignals} chap=${rep.auditAfter.chapterSignals} ticsFS=${rep.auditAfter.ticsFailShadow} unpaid=${rep.auditAfter.seedsUnpaid} **coutures=${rep.auditAfter.brokenStitches}**`,
    `## PREUVE CIBLÉE (NCR-C11-001) — chaque réparation, compte exact avant→après:`,
    ...rep.targetedProof.map((p) => `- ${p.label} : ${p.before} → ${p.after}`),
    `## Dérives RÉSIDUELLES (pourquoi drifts reste >0 — bruit advisory nommé):`,
    ...rep.residualDrifts.map((d) => `- « ${d.role} » : ${d.names.join(' / ')}`),
    `Lecture des compteurs stables : physics inclut le FOOTWEAR (SURGICAL préparé NON exécuté — GO_B) + 5 DOOR (SIGNAL_ONLY) ; chap/ticsFS/unpaid = SIGNAL_ONLY jamais réparés automatiquement PAR DOCTRINE.`,
    `Actions appliquées:`,
