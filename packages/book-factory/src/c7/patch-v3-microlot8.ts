/**
 * OMEGA — RUNNER MICRO-LOT 8 (corrections auteur Francky 2026-06-12 sur micro-lot 7).
 * Identique au runner 7 ; seules 2 substitutions changent (A096 ch.18 reformulée option A,
 * A279 ch.49 « la paume »→« sa paume »). TRIPLE garde inchangée, REVERT bit-identique,
 * le canon (MANUSCRIT_V3_PATCHED.md) n'est JAMAIS écrasé : sortie = NOUVEAU fichier.
 *   tsx packages/book-factory/src/c7/patch-v3-microlot8.ts   (cwd = packages/book-factory)
 */
...
const OPS_FILE = `${OUT}/MICROLOT8_OPS.json`;
const OUT_MS = `${OUT}/MANUSCRIT_V3_MICROLOT8.md`;
const LEDGER = `${OUT}/AP_MICROLOT8_LEDGER.jsonl`;
const SCAN = `${OUT}/AP_MICROLOT8_REPETITION_SCAN.md`;
...
async function main(): Promise<void> {
  ...
  const scanLines = ['# AP_MICROLOT8 — scan répétition book-wide (avant/après)', '',
  ...
  console.log(`MICROLOT8_DONE applied=${applied}/${ops.length}${halted ? ` HALT=${halted}` : ''} canonFinal=${finalBuild.ok ? finalBuild.value.finalHash : 'FAIL'}`);
}
