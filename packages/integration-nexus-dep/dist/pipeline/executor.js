...
export class PipelineExecutor {
    ...
    async execute(definition, input) {
        ...
        try {
            ...
            for (let i = 0; i < definition.stages.length; i++) {
                ...
                // Check dependencies
                if (stage.dependsOn) {
                    const unmetDeps = stage.dependsOn.filter(dep => !stageResults.some(r => r.stageName === dep && r.status === "completed"));
                    if (unmetDeps.length > 0) {
                        const error = createNexusError("VALIDATION_FAILED", `Unmet dependencies for stage ${stage.name}: ${unmetDeps.join(", ")}`, "pipeline");
                ...
                const stageResult = await this.executeStage(stage, currentOutput, {
                    pipelineId,
     ...
     */
    async executeStage(stage, input, context, options) {
        const startTimeMs = Date.now();
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                const result = await this.executeWithTimeout(() => stage.handler(input, context), timeoutMs);
                const endTimeMs = Date.now();
                this.emit({
                    type: "stage:complete",
                    pipelineId: context.pipelineId,
