/**
 * OMEGA Runner — CLI: run create
 * Phase D.1 — IntentPack -> CreationResult
 */
...
/** Execute the run-create command */
export function executeRunCreate(args: ParsedArgs): number {
  ...
  try {
    ...
    const result = orchestrateCreate(intent, seed, timestamp, logger);
    ...
    const finalHash = creation.output_hash;
    const verdict = creation.verdict;
