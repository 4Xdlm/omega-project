...
import type { DeterminismDifference, DeterminismReport, RunResult } from './types.js';
 ...
 */
function findDifferences(v1: unknown, v2: unknown, path: string): DeterminismDifference[] {
 ...
 */
function normalizeForComparison(result: RunResult): Record<string, unknown> {
 ...
 */
export class DefaultDeterminismGuard implements DeterminismGuard {
  /**
 ...
 */
export function createDeterminismGuard(): DeterminismGuard {
 ...
 */
export function assertDeterministic(run1: RunResult, run2: RunResult): void {
  ...
  if (!report.is_deterministic) {
    const diffSummary = report.differences
      .slice(0, 5)
      .map((d) => `  ${d.path}: ${stableStringify(d.value1)} !== ${stableStringify(d.value2)}`)
      .join('\n');
    throw new Error(
