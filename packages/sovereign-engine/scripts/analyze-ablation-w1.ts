...
// Trouver le dernier ValidationPack W1
...
console.log(`  Verdict:              ${globalSealRate >= 25 ? 'TARGET ATTEINT (>=25%)' : sealRateDelta > 0 ? 'PROGRESSION INSUFFISANTE' : 'REGRESSION'}`);
console.log('');
...
const report = {
  ...
  verdict: globalSealRate >= 25 ? 'TARGET_ATTEINT' : sealRateDelta > 0 ? 'PROGRESSION' : 'REGRESSION',
