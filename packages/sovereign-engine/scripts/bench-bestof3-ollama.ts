/**
 * OMEGA — BEST-OF-3 VALIDATION BENCH — FULL OLLAMA
 * Même pipeline que Anthropic (juges + corrections + scoring) mais 100% Ollama.
 * Compare directement avec les résultats Anthropic (comp=89.9, min=83.8).
 * 
 * Usage : cd packages/sovereign-engine && npx tsx scripts/bench-bestof3-ollama.ts
 * Env : OMEGA_OLLAMA_MODEL=qwen3:32b (ou qwen3.5:35b-a3b, mistral-small:24b)
 */
...
async function main() {
  ...
  const ANTHROPIC_BO3 = { comp: 89.9, cliff: 0.377, min_axis: 83.8, saga_pct: 9 };
  ...
  console.log(`\nVERDICT BEST-OF-3 OLLAMA : ${allPass ? '✅ PASS' : '❌ FAIL'}`);

  // ═══ COMPARAISON OLLAMA vs ANTHROPIC ═══
  console.log('\n═══════════════════════════════════════════════════');
  console.log('COMPARAISON OLLAMA vs ANTHROPIC (même pipeline)');
  console.log('═══════════════════════════════════════════════════');
  console.log(`|                | Anthropic BO3 | Ollama BO3 (${ollamaModel}) | Delta |`);
  console.log(`|----------------|---------------|${'-'.repeat(ollamaModel.length + 15)}|-------|`);
  console.log(`| Composite      | ${ANTHROPIC_BO3.comp.toFixed(1).padStart(13)} | ${avgComp.toFixed(1).padStart(ollamaModel.length + 13)} | ${(avgComp - ANTHROPIC_BO3.comp > 0 ? '+' : '') + (avgComp - ANTHROPIC_BO3.comp).toFixed(1).padStart(5)} |`);
  console.log(`| min_axis       | ${ANTHROPIC_BO3.min_axis.toFixed(1).padStart(13)} | ${avgMin.toFixed(1).padStart(ollamaModel.length + 13)} | ${(avgMin - ANTHROPIC_BO3.min_axis > 0 ? '+' : '') + (avgMin - ANTHROPIC_BO3.min_axis).toFixed(1).padStart(5)} |`);
  console.log(`| cliff          | ${ANTHROPIC_BO3.cliff.toFixed(3).padStart(13)} | ${avgCliff.toFixed(3).padStart(ollamaModel.length + 13)} | ${(avgCliff - ANTHROPIC_BO3.cliff > 0 ? '+' : '') + (avgCliff - ANTHROPIC_BO3.cliff).toFixed(3).padStart(5)} |`);
  console.log(`| SAGA %         | ${(ANTHROPIC_BO3.saga_pct + '%').padStart(13)} | ${(sagaPct + '%').padStart(ollamaModel.length + 13)} | ${(sagaPct - ANTHROPIC_BO3.saga_pct > 0 ? '+' : '') + (sagaPct - ANTHROPIC_BO3.saga_pct) + '%'.padStart(4)} |`);
  console.log(`| crashes (<70)  | ${'0'.padStart(13)} | ${String(crashCount).padStart(ollamaModel.length + 13)} |       |`);
