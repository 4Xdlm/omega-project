...
async function main(): Promise<void> {
  ...
  for (const seed of seeds) {
    ...
    try {
      ...
      const isProviderFail = errMsg.includes('credit balance') || errMsg.includes('rate limit') || errMsg.includes('quota') || errMsg.includes('billing') || errMsg.includes('overloaded') || errMsg.includes('Overloaded') || errMsg.includes('529') || errMsg.includes('not_found') || errMsg.includes('404') || errMsg.includes('authentication_error') || errMsg.includes('401') || errMsg.includes('invalid x-api-key');
      const errorJSON = {
        schema: 'omnipotent.calibration.v1',
        seed,
        provider,
