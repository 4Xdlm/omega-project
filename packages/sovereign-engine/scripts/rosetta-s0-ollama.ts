...
// [OLLAMA ADAPTER 2026-06-04] Anthropic remplace par Ollama local (gemma4:31b). Cf CAL-B/EMP-19.
...
let apiCalls = 0;
const MAX_API_CALLS = Number(process.env['ROSETTA_MAX_CALLS'] ?? 400);
...
async function callLLM(client: Anthropic, prompt: string, maxTokens = 1024): Promise<string> {
  ...
  try {
    ...
    const r = await fetch('http://localhost:11434/api/generate', {
      ...
      body: JSON.stringify({ model: MODEL, prompt, stream: false, think: false, options: { temperature: 0, num_predict: maxTokens } }),
    });
...
function buildRosettaMatrix(): void {
  ...
  const matrix: Record<string, unknown> = {
    matrix_id: `rosetta_gemma4-31b_v1`,
    model: MODEL,
  ...
  writeJSON(path.join(S0_DIR, `rosetta_gemma4-31b_v1.json`), matrix);
}
...
async function main(): Promise<void> {
  const client = {} as Anthropic; // Ollama : pas de client API
