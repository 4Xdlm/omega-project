/**
 * run-v5-bench-mini.ts — Mini bench V4 vs V5.1 (Bridge-02: 5 features)
 * 4 scènes seulement : contemplation, menace, confrontation, passion
 * Objectif : mesurer l'impact de f29d (TTR bande) + f35c (hook)
 *
 * Usage (PowerShell) :
 *   npx tsx scripts/run-v5-bench-mini.ts
 *
...
// ── 4 scènes (internal×2, external×1, societal×1) ──────────────────────
...
function buildPacket(scene: typeof SCENES[0], runIdx: number, version: 'v4' | 'v5'): ForgePacket {
  return {
    packet_id: `BENCH_BR02_${scene.id}_${version}_${runIdx}`,
    packet_hash: 'a'.repeat(64),
    scene_id: `bench_br02_${scene.id}`,
    run_id: `bench_br02_${scene.id}_${version}_${runIdx}_${Date.now()}`,
    ...
    seeds: { llm_seed: `bench_br02_${scene.id}_${runIdx}_${Date.now()}`, determinism_level: 'absolute' },
...
async function main(): Promise<void> {
  ...
  console.log(`\n[BR02-BENCH] ═══════════════════════════════════════════════════════`);
  console.log(`[BR02-BENCH]  OMEGA V4 vs V5.1 (Bridge-02) MINI BENCH — ${SCENES.length} scenes`);
  console.log(`[BR02-BENCH]  Phase 2 features: contrast + compression + rarity + TTR + hook`);
  console.log(`[BR02-BENCH]  Model: ${MODEL} | Temp: draft=${DRAFT_TEMPERATURE} judge=${JUDGE_TEMPERATURE}`);
  console.log(`[BR02-BENCH] ═══════════════════════════════════════════════════════\n`);

  for (let i = 0; i < SCENES.length; i++) {
    const scene = SCENES[i];
    console.log(`\n[BR02-BENCH] ─── Scene ${i + 1}/${SCENES.length}: ${scene.id} (${scene.conflict}) ───\n`);

    // ── V4 ──
    console.log(`[BR02-BENCH] Running V4...`);
    ...
    try {
      ...
      console.log(`[BR02-BENCH] V4 ${scene.id}: composite=${score.composite.toFixed(1)} ecc=${score.ecc.toFixed(1)} rci=${score.rci.toFixed(1)} sii=${score.sii.toFixed(1)} loops=${score.loop_passes} words=${score.word_count}`);
    } catch (err: any) {
      const msg = err.message?.slice(0, 200) ?? String(err);
      console.error(`[BR02-BENCH] V4 ${scene.id} FAILED: ${msg}`);
    ...
    // ── V5.1 ──
    console.log(`[BR02-BENCH] Running V5.1 (Bridge-02)...`);
    ...
    try {
      ...
      console.log(`[BR02-BENCH] V5.1 ${scene.id}: composite=${score.composite.toFixed(1)} ecc=${score.ecc.toFixed(1)} rci=${score.rci.toFixed(1)} sii=${score.sii.toFixed(1)} loops=${score.loop_passes} words=${score.word_count}`);
    } catch (err: any) {
      const msg = err.message?.slice(0, 200) ?? String(err);
      console.error(`[BR02-BENCH] V5.1 ${scene.id} FAILED: ${msg}`);
      errors.push({ scene: scene.id, version: 'V5.1', error: msg });
    ...
    if (v4c > 0 || v5c > 0) {
      console.log(`[BR02-BENCH] Delta ${scene.id}: ${fmtDelta(v5c - v4c).trim()}`);
    }

    // Pause entre scènes pour rate limiting
    if (i < SCENES.length - 1) {
      console.log(`[BR02-BENCH] Pause 3s...`);
  ...
  console.log(`\n[BR02-BENCH] ═══ TABLEAU COMPARATIF ═══\n`);
  console.log('| Scene           | V4 comp | V5.1 comp |  Delta | V4 ECC | V5 ECC | V4 loop | V5 loop | V4 words | V5 words |');
  console.log('|-----------------|---------|-----------|--------|--------|--------|---------|---------|----------|----------|');
  ...
  console.log(
    ...
    `| ${fmt(mean(v4Valid.map(s => s.ecc)))} ` +
    `| ${fmt(mean(v5Valid.map(s => s.ecc)))} ` +
  ...
  console.log(`\n[BR02-BENCH] Scenes valides: V4=${v4Valid.length}/${SCENES.length} V5.1=${v5Valid.length}/${SCENES.length}`);

  // Axes détaillés
  console.log(`\n| Axe  | V4 mean | V5.1 mean |  Delta |`);
  console.log(`|------|---------|-----------|--------|`);
  ...
  if (errors.length > 0) {
    console.log(`\n[BR02-BENCH] ═══ ERRORS (${errors.length}) ═══`);
    for (const e of errors) {
      console.log(`[BR02-BENCH] ${e.version} ${e.scene}: ${e.error}`);
    }
  }

  // ── VERDICT ──────────────────────────────────────────────────────────────

  const delta = v5MeanComp - v4MeanComp;
  const v4Loops = mean(v4Valid.map(s => s.loop_passes));
  const v5Loops = mean(v5Valid.map(s => s.loop_passes));
  const eccDelta = mean(v5Valid.map(s => s.ecc)) - mean(v4Valid.map(s => s.ecc));

  let verdict: string;
  let reason: string;

  if (v4Valid.length < 2 || v5Valid.length < 2) {
    verdict = 'INCONCLUSIVE';
    reason = `Donnees insuffisantes: V4=${v4Valid.length} V5.1=${v5Valid.length} valides (min 2)`;
  } else if (delta < -0.5) {
    verdict = 'NO_GO';
    reason = `Regression composite: V5.1 (${v5MeanComp.toFixed(1)}) < V4 (${v4MeanComp.toFixed(1)}) de ${Math.abs(delta).toFixed(1)}`;
  } else if (eccDelta < -2.0) {
    verdict = 'ALERTE_ECC';
    reason = `ECC en chute: delta=${eccDelta.toFixed(1)} (seuil -2.0). Composite OK (${delta >= 0 ? '+' : ''}${delta.toFixed(1)}) mais axe emotionnel degrade`;
  } else if (delta >= 0 && v5Loops <= v4Loops) {
    verdict = 'GO';
    reason = `V5.1 (${v5MeanComp.toFixed(1)}) >= V4 (${v4MeanComp.toFixed(1)}), loops V5.1 (${v5Loops.toFixed(1)}) <= V4 (${v4Loops.toFixed(1)})`;
  } else {
    verdict = 'GO_CONDITIONNEL';
    reason = `V5.1 (${v5MeanComp.toFixed(1)}) vs V4 (${v4MeanComp.toFixed(1)}): delta=${delta >= 0 ? '+' : ''}${delta.toFixed(1)}, loops V5.1=${v5Loops.toFixed(1)} vs V4=${v4Loops.toFixed(1)}`;
  }

  console.log(`\n[BR02-BENCH] ═══ VERDICT ═══`);
  console.log(`[BR02-BENCH] ${verdict}`);
  console.log(`[BR02-BENCH] Raison: ${reason}`);
  if (verdict === 'ALERTE_ECC') {
    console.log(`[BR02-BENCH] ACTION: Verifier si f29d ou f35c degrade la coherence emotionnelle`);
  }
  ...
  const benchResult = {
    bench_id: `BENCH_BR02_MINI_${timestamp.replace(/[-T]/g, '')}`,
    protocol: 'V4 vs V5.1 (Bridge-02) — 4 scenes, meme seed/modele/temperature',
    bridge_02_features: ['f24e_contrast_score', 'f15b_redundancy_compression', 'f16a_bigram_rarity', 'f29d_ttr_score', 'f35c_hook_score'],
    model: MODEL,
    ...
    prompt_assembler_version: '5.1.0',
    standard: 'NASA-Grade L4 / DO-178C Level A',
  };

  const outPath = resolve(sessionsDir, `br02-bench-${timestamp}.json`);
  writeFileSync(outPath, JSON.stringify(benchResult, null, 2), 'utf-8');
  console.log(`\n[BR02-BENCH] Resultats: ${outPath}`);

  const prosePath = resolve(sessionsDir, `br02-bench-prose-${timestamp}.json`);
  ...
  console.log(`[BR02-BENCH] Prose: ${prosePath}`);
...
main().catch(err => {
  console.error('[BR02-BENCH] FATAL:', err);
  process.exit(1);
});
