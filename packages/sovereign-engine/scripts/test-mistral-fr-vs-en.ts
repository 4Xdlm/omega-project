/**
 * OMEGA — Test Mistral FR vs EN — Language Bottleneck
 * 4 scenes × 2 languages = 8 API calls (Mistral only)
 *
 * Usage: MISTRAL_API_KEY=... npx tsx scripts/test-mistral-fr-vs-en.ts
 */
...
async function generateMistral(apiKey: string, prompt: string): Promise<string> {
  const res = await fetch('https://api.mistral.ai/v1/chat/completions', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}` },
    body: JSON.stringify({ model: MISTRAL_MODEL, messages: [{ role: 'user', content: prompt }], max_tokens: MAX_TOKENS, temperature: TEMPERATURE }),
  });
...
async function main() {
  ...
  console.log('  OMEGA — MISTRAL FR vs EN — LANGUAGE BOTTLENECK');
  console.log(`  Model: ${MISTRAL_MODEL} | Temp: ${TEMPERATURE} | Scenes: ${SCENES.length}`);
  console.log('=' .repeat(65));

  for (const scene of SCENES) {
    console.log(`\n[${scene.id}] FR...`);
    const pFR = await withRetry(() => generateMistral(mistralKey, scene.fr), `FR ${scene.id}`);
    ...
    console.log(`[${scene.id}] EN...`);
    const pEN = await withRetry(() => generateMistral(mistralKey, scene.en), `EN ${scene.id}`);
  ...
  // Summary
  const frResults = results.filter(r => r.lang === 'FR');
  const enResults = results.filter(r => r.lang === 'EN');
  const mgb = mean(enResults.map(r => r.gb)) - mean(frResults.map(r => r.gb));
  const mcv = mean(enResults.map(r => r.cv)) - mean(frResults.map(r => r.cv));
  const mf26 = mean(enResults.map(r => r.f26b)) - mean(frResults.map(r => r.f26b));

  console.log(`\n${'='.repeat(65)}`);
  console.log('  SUMMARY');
  console.log(`${'='.repeat(65)}`);
  console.log(`  Mistral FR mean GB: ${mean(frResults.map(r => r.gb)).toFixed(3)}`);
  console.log(`  Mistral EN mean GB: ${mean(enResults.map(r => r.gb)).toFixed(3)}`);
  console.log(`  Delta GB: ${mgb > 0 ? '+' : ''}${mgb.toFixed(3)}`);
  console.log(`  Delta CV: ${mcv > 0 ? '+' : ''}${mcv.toFixed(3)}`);
  console.log(`  Delta f26b: ${mf26 > 0 ? '+' : ''}${mf26.toFixed(4)}`);

  console.log('\n  CROSS-MODEL (reference Claude FR vs EN):');
  console.log('  Claude: ΔGB=+0.099');
  console.log(`  Mistral: ΔGB=${mgb > 0 ? '+' : ''}${mgb.toFixed(3)}`);

  const verdict = Math.abs(mgb) <= 0.05 ? 'NO_BOTTLENECK' : mgb > 0.05 ? 'BOTTLENECK_UNIVERSAL' : 'FR_ADVANTAGE';
  console.log(`\n  VERDICT: ${verdict}`);
  if (verdict === 'NO_BOTTLENECK') console.log('  → Mistral has NO language bottleneck. The issue is Claude-specific (RLHF).');
  else if (verdict === 'BOTTLENECK_UNIVERSAL') console.log('  → ALL LLMs have this bottleneck. Fix the prompt, not the model.');
  else console.log('  → Mistral is BETTER in French (unexpected).');

  // Save
  const dataDir = resolve(__dirname, '../src/scoring/data');
  writeFileSync(join(dataDir, 'MISTRAL_FR_VS_EN_BOTTLENECK.json'), JSON.stringify({
    date: new Date().toISOString(), model: MISTRAL_MODEL, temperature: TEMPERATURE,
    verdict, mean_delta_gb: r4(mgb), mean_delta_cv: r4(mcv), mean_delta_f26b: r4(mf26),
    claude_reference_delta_gb: 0.099,
    results: results.map(({ prose: _, ...rest }) => rest),
  }, null, 2));

  const proseDir = resolve(__dirname, `../sessions/MISTRAL_FR_VS_EN_${ts}`);
  mkdirSync(proseDir, { recursive: true });
  for (const r of results) writeFileSync(join(proseDir, `${r.scene}_${r.lang}.txt`), r.prose);

  console.log(`\n  Saved: data/MISTRAL_FR_VS_EN_BOTTLENECK.json + ${proseDir}`);
  console.log('=' .repeat(65));
}
