...
describe('RCI hook verification', () => {
  it('INV-RCI-HOOKS-01: RCI includes hook_presence sub-score with weight 0.20', () => {
    const prose = `The iron gates stood silent. Shadows crept along metal walls.`;
    const result = computeRCI(MOCK_PACKET, prose);
  ...
  it('all hooks present in prose → high hook_presence score', () => {
  ...
  it('no hooks present in prose → low hook_presence score', () => {
  ...
  it('RCI weight blend: rhythm×0.45 + signature×0.35 + hooks×0.20', () => {
