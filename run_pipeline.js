// ═══════════════════════════════════════════════════════════════════════════════
// OMEGA PIPELINE RUNNER v1.0.0 (ESM)
// Usage: node run_pipeline.js <input_file> [output_file]
// ═══════════════════════════════════════════════════════════════════════════════
...
async function main() {
  ...
  if (args.length < 1) {
    console.log('Usage: node run_pipeline.js <input_file> [output_file]');
    process.exit(1);
  }
  ...
  // 1. Charger dump_analysis.json existant
  console.log('\n[1/3] Loading analysis...');
  const dumpPath = path.join(__dirname, 'dump_analysis.json');
  
  if (!fs.existsSync(dumpPath)) {
    console.error('ERROR: dump_analysis.json not found. Run TextAnalyzer first.');
    process.exit(1);
  }
  ...
  // 2. Bridge: Analysis -> Segments
  console.log('\n[2/3] Building bridge data...');
  const bridge = await import('./packages/omega-bridge-ta-mycelium/src/bridge/index.js');
  
  const dnaInputs = bridge.prepareDNABuild(analysis, { seed: SEED, title: path.basename(inputFile) });
  const validation = bridge.validateDNAInputs(dnaInputs);
  ...
  const mycelium = await import('./packages/mycelium-bio/src/index.js');
  
  const dna = mycelium.buildMyceliumDNA(dnaInputs.segments, dnaInputs.options);
