...
// ═══════════════════════════════════════════════════════════════════════════════
// GENOME MODULE ATTACKS (Sprint 28.5)
// Target: INV-GEN-* invariants from @omega/genome v1.2.0
// ═══════════════════════════════════════════════════════════════════════════════

const GENOME_ATTACKS: readonly AttackDefinition[] = Object.freeze([
  {
    id: 'ATK-GEN-001',
    name: 'JSON Key Permutation',
    category: 'semantic',
    description: 'Permute JSON keys and verify fingerprint changes',
    successCriteria: 'Canonical serialization produces identical fingerprint regardless of key order',
    failureCriteria: 'Fingerprint changes with key permutation',
    severity: 'CRITICAL',
    tags: ['genome', 'canonical', 'fingerprint', 'INV-GEN-13'],
    mandatory: true
  },
  {
    id: 'ATK-GEN-002',
    name: 'Float Drift Attack',
    category: 'semantic',
    description: 'Inject float values differing by less than 1e-6',
    successCriteria: 'Quantization at 1e-6 produces identical fingerprint',
    failureCriteria: 'Float precision causes fingerprint drift',
    severity: 'CRITICAL',
    tags: ['genome', 'float', 'quantization', 'INV-GEN-14'],
    mandatory: true
  },
  {
    id: 'ATK-GEN-003',
    ...
    category: 'semantic',
    description: 'Inject/modify metadata fields and verify fingerprint unchanged',
    successCriteria: 'Metadata excluded from fingerprint computation',
    failureCriteria: 'Metadata changes fingerprint',
    severity: 'CRITICAL',
    tags: ['genome', 'metadata', 'fingerprint', 'INV-GEN-11'],
    mandatory: true
  },
  {
    id: 'ATK-GEN-004',
    name: 'Emotion14 Length Violation',
    category: 'semantic',
    description: 'Send emotion distribution with wrong count (13 or 15)',
    successCriteria: 'Validation rejects non-14 emotion distributions',
    failureCriteria: 'System accepts invalid emotion count',
    severity: 'CRITICAL',
    tags: ['genome', 'emotion14', 'validation', 'INV-GEN-12'],
    mandatory: true
  },
  {
    id: 'ATK-GEN-005',
    name: 'Distribution Sum Violation',
    category: 'semantic',
    description: 'Send emotion distribution that sums to != 1.0',
    successCriteria: 'Validation rejects or normalizes to 1.0',
    failureCriteria: 'Invalid distribution accepted without normalization',
    severity: 'HIGH',
    tags: ['genome', 'distribution', 'validation', 'INV-GEN-04'],
    mandatory: true
  }
]);
