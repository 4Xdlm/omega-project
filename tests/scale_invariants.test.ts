// ═══════════════════════════════════════════════════════════════════════════════
// OMEGA SCALE — INVARIANTS TEST SUITE v1.0.0
// ═══════════════════════════════════════════════════════════════════════════════
...
La colère montait en lui, irrépressible et violente.
Un sentiment de tristesse l'envahit lentement mais sûrement.

###

L'espoir renaissait, fragile mais tenace comme une flamme.
Il était surpris par cette révélation soudaine et inattendue.
La confiance qu'il avait en elle était inébranlable.

Le dégoût se lisait sur son visage pâle et crispé.
L'amour qu'ils partageaient était plus fort que tout obstacle.
La honte le submergea, brûlante et cuisante comme une plaie.

###

Il ressentait une fierté immense devant son accomplissement final.
Le désespoir s'abattit sur elle comme une chape de plomb froide.
Mais au fond de son cœur, une lueur persistait encore.
`.trim();
...
describe("OMEGA SCALE — Invariants L4", () => {
  beforeAll(() => {
    setupTestDir();
  });
  ...
  describe("Stress Tests", () => {
    ...
    it("batch summary is created correctly", () => {
      ...
      fs.writeFileSync(path.join(batchDir, "file3.txt"), "Colère intense!", "utf8");
