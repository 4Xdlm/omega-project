...
import { mustReadUtf8, writeUtf8, fileExists } from "../lib/io.js";
...
const OUTPUT_DIR = "nexus/proof/phase_b/b3";
...
async function main() {
  ...
  const report = { phase: "B3", inputs: hashes, determinism: { B1_match: b1Match, B2_match: b2Match }, signature, verdict: b1Match && b2Match ? "PASS" : "FAIL" };

  writeUtf8(`${OUTPUT_DIR}/B3_CROSSRUN_REPORT.json`, canonicalJson(report));
  writeUtf8(`${OUTPUT_DIR}/B3_SIGNATURE_DIGEST.txt`, `B3_SIGNATURE_SHA256 ${signature}`);
  writeUtf8(`${OUTPUT_DIR}/B3_CROSSRUN_REPORT.md`, `# B3 Cross-Validation\n\nVerdict: ${report.verdict}\nB1 Match: ${b1Match}\nB2 Match: ${b2Match}\nSignature: ${signature}`);
